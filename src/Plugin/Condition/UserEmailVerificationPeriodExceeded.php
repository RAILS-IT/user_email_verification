...
/**
 * Provides a 'User email verification period exceeded' condition.
 *
 * @Condition(
 *   id = "user_email_verification_period_exceeded",
 *   label = @Translation("User email verification period exceeded"),
 ...
 * )
 */
class UserEmailVerificationPeriodExceeded extends RulesConditionBase implements ContainerFactoryPluginInterface {
  ...
  /**
   * Constructs a UserEmailVerificationPeriodExceeded object.
   *
  ...
  /**
   * Check if user email verification period exceeded.
   *
   * @param \Drupal\user\UserInterface $user
   *   The account to check.
   *
   * @return bool
   *   TRUE if user email verification period exceeded.
   */
  protected function doEvaluate(UserInterface $user) {
    return !$this->userEmailVerification->isVerificationPeriodExceeded($user->id());
  }
